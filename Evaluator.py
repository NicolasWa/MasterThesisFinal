    ...
    for y in range(nb_tiles_y):
        for x in range(nb_tiles_x):
            ...
            if tile_tissue_mask.sum() > min_tissue_area:
                ...
                X[0] = tissue_tile
                pred_tissue_tile = model.predict(X)[0]  
                pred_tile_prob = pred_tissue_tile[:, :,1]  
                Y_pred_prob[y_up:y_down, x_left:x_right] = pred_tile_prob 
            else:
...
def predict_WSI_BASIC(model, img, tissue_mask, Y_gt, tile_size, min_tissue_percentage_area=0.3, overlap=False):
    ...
    if (Y_pred_prob.shape[0]>31600 or Y_pred_prob.shape[1]>31600): #requires too much memory (only happens for the biggest file) -> put a dummy prob map
        ...
        Y_pred_prob_downscaled = downscale_local_mean(Y_pred_prob, (4, 4)) # to downscale with a factor 4
    ...
    save_probmap(Y_pred_prob_downscaled[upper_left[0]:lower_right[0], upper_left[1]:lower_right[1]], os.path.join(save_folder_path, name + "_cropped_prob_map.png"))
    return Y_pred_mask.astype("uint8"), Y_pred_prob_downscaled 
...
class Evaluator:
    ...
    def predict_WSI_and_evaluate_BASIC(cls, model, path_test_folder, path_pred_folder, tile_size, mag_factor = 10, overlap = False):
        ...
        for X_path, Y_path in zip(X_files_path, Y_files_path):
            ...
            save_img(X, path_name + "_img.jpeg") 
            ...
            save_probmap(Y_pred_prob_downscaled, path_name + "_prob_map.png")
...
def test_pred():
    ...
    path_to_model = os.path.join(os.getcwd(),"models", clf_name) 
    ...
    path_pred_folder = os.path.join(os.getcwd(), "test_folder", pred_folder_name)
...
def small_test():
    ...
    path_pred_folder = os.path.join(os.path.dirname(os.getcwd()), "DataThesis", "predictions")
    path_metrics_folder = path_pred_folder  
    model = Model(tile_size, path_to_model, loadFrom=path_to_model)

def test_model_on_test_set(test_conditions):
    mag = test_conditions['mag']
    tile_size = test_conditions['tile_size']
    overlap_pred = test_conditions['overlap_pred'] 
    experiment_model = test_conditions['experiment_model']
    path_test_folder = test_conditions['path_test_folder']
    ...
    path_pred_model_folder = os.path.join(path_predictions_folder, pred_folder_name) 
...
def retrieve_cropped_imgs():
    ...
    apath = os.path.join(folder_path, name+".ndpa")  
